...
// *************************************************************************
  ...
  class Core
  {
      ...
      public function getCourier_list()
      {
	  	...
	  	$sql = "SELECT a.*, s.mod_style, s.color  FROM add_courier a, styles s WHERE a.status_courier=s.mod_style  AND a.con_status='0' AND a.status_novelty='0'";
	  	if(isset($_GET['username']) && !empty($_GET['username']))
	  	{
	  		$sql = $sql." AND a.username='".sanitize($_GET['username'])."'"; 
	  	}
      ...
      public static function getRowByName($table, $name, $and = false, $is_admin = true)
      {
          $name = sanitize($name);
          if ($and) {
              $sql = "SELECT * FROM " . (string )$table . " WHERE username = '" . (string)$name . "' AND " . Registry::get("Database")->escape($and) . "";
          } else
              $sql = "SELECT * FROM " . (string)$table . " WHERE username = '" . (string)$name . "'";
